...
void HuffmanCode(HuffmanNode* hufmTree, int depth)      // depth是哈夫曼树的深度
{
    ...
    if (hufmTree)
    {
        if (hufmTree->lchild==NULL && hufmTree->rchild==NULL)
        {
            printf("id:%d weigh: %d encode:", hufmTree->id, hufmTree->weight);
            int i;
            for (i=0; i<depth; ++i)
            {
                printf("%d", code[i]);
...
int main()
{
    ...
    printf("input %d the each weights of leaf nodes:\n", n);
    for (i=0; i<n; ++i)
    {
        scanf("%d", &arr[i]);
    }
    ...
    printf("input %d characters represented by the leaf nodes:\n", n);
    ...
    printf("The generalized table form of this Huffman tree is:\n");
    PrintHuffmanTree(hufmTree);
    printf("\nThe Huffman code for each leaf node is:\n");
    HuffmanCode(hufmTree, 0);

    printf("wanna decode, input the codes:\n");
    gets(ch);
